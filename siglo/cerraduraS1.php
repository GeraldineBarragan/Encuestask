...
// Inicializar $json con un valor por defecto
$json = ["codigo" => 0, "estatus" => 1, "msg" => "REQUEST_METHOD NO ACEPTADO"];
    ...
    if ($id != '' || $operador != '') {
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($resultFechadura && $resultFechadura->num_rows > 0) {
                    while ($rowFechadura = $resultFechadura->fetch_assoc()) {
                        $ns = $rowFechadura["ns"];
                        $idFe = $rowFechadura["id"];

                        $urlApertura =
                            "http://localhost:8091/siglotech/index.php/api?debug=1"
                            . "&key="     . $apikey
                        ...
                        switch ($tipo) {
                            case "1": // Apertura
                                ...
                                if ($jsonFechadura && isset($jsonFechadura["sts"])) {
                                    ...
                                    $json = ["codigo" => "", "estatus" => 7, "msg" => "RESPUESTA DE API INVALIDA"];
                                }
                                break;
                            ...
                            default:
                                $json = ["codigo" => "", "estatus" => 8, "msg" => "TIPO DE OPERACION NO VALIDO"];
                                break;
                        }
                    }
                } else {
